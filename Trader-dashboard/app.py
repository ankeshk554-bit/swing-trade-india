...
from core.shoonya_bridge import (
    ...
    load_data_shoonya, load_credentials, save_credentials,
...
[data-testid="stSidebar"] { background-color: #0c1220; border-right: 1px solid #1f2b37; }
...
/* Progress bar */
.stProgress > div > div > div { background-color: #f5c542; }

/* Signal badges */
.buy-tag { color: #00ff9f; font-weight: bold; background: rgba(0,255,159,0.1); padding: 2px 10px; border-radius: 4px; }
.sell-tag { color: #ff4d6d; font-weight: bold; background: rgba(255,77,109,0.1); padding: 2px 10px; border-radius: 4px; }
.neutral-tag { color: #f5c542; font-weight: bold; background: rgba(245,197,66,0.1); padding: 2px 10px; border-radius: 4px; }
...
.card-title { color: #f5c542; font-size: 14px; font-weight: 600; margin-bottom: 8px; }
.card-value { font-size: 24px; font-weight: bold; color: white; }
.card-sub { font-size: 12px; color: #6b7280; margin-top: 4px; }

/* Expander */
.streamlit-expanderHeader { background-color: #111827 !important; border-radius: 8px; }
</style>
...
with st.sidebar.expander("⚙️ Settings"):
    ...
    if not saved_creds:
        ...
        shoonya_2fa = st.text_input("2FA (PAN+DOB or TOTP)", "", type="password", key="sh_2fa",
                                    help="Usually PAN in uppercase + DOB (DDMMYYYY)")
        if st.button("🔗 Connect Shoonya", key="sh_connect"):
            if shoonya_uid and shoonya_pwd and shoonya_2fa:
                with st.spinner("Connecting..."):
                    if shoonya_connect(shoonya_uid, shoonya_pwd, shoonya_2fa):
...
st.caption(
    "India's Institutional-Grade Swing Trading Platform — "
    "RS Rating • Smart Money • Regime Trading • Multi-Timeframe"
)

# Market Regime Banner (with fallback for rate limits)
try:
...
with cols[0]:
    st.metric(
        f"{regime_emoji} Market Regime",
        f"{regime} ({strength})",
        f"Nifty: {regime_info.get('nifty_close', '—')}",
        delta_color="off"
    )
with cols[1]:
    st.metric("RSI", regime_info.get("rsi", "—"), f"Zone: {regime_info.get('rsi_zone', '—')}")
with cols[2]:
    st.metric("ADX", regime_info.get("adx", "—"), f"Trend Strength")
with cols[3]:
    st.metric("Nifty vs EMA50", f"{regime_info.get('ema50_distance', 0):+.1f}%")
with cols[4]:
    st.metric("Nifty vs EMA200", f"{regime_info.get('ema200_distance', 0):+.1f}%")
...
# =========================================================
    ...
    scan_button = st.button(f"🚀 Run Scan{' (Deep)' if include_fundamental else ''}", use_container_width=True)
...
# =========================================================
    ...
    with chart_tab2:
        ...
        if st.button("🔄 Compare", key="comp_btn", use_container_width=True):
    ...
    with chart_tab3:
        ...
        if st.button("📊 Generate Volume Profile", key="vp_gen", use_container_width=True):
...
# =========================================================
    ...
    with atab1:
        ...
        if st.button("🔍 Detect Levels", key="sr_detect", use_container_width=True):
    ...
    with atab2:
        ...
        if st.button("🔄 Compare Strategies", key="comp_run", use_container_width=True) and selected_strats:
    ...
    with atab3:
        ...
        with ai_col2:
            if st.button("🧠 Score Signal", key="ai_score", use_container_width=True):
    ...
    with atab4:
        ...
        if fib_method == "Manual Entry":
            ...
            if st.button("📐 Calculate Fib", key="fib_calc", use_container_width=True):
            ...
            if st.button("🔍 Auto Detect Swing Points", key="fib_auto", use_container_width=True):
    ...
    with exec_tab2:
        ...
        with jtab2:
            ...
            if not all_trades_df.empty:
                ...
                st.dataframe(all_display[display_cols], use_container_width=True, hide_index=True,
                             column_config={
    ...
    with exec_tab4:
        ...
        if st.button("🧮 Calculate", key="ps_calc", use_container_width=True):
    ...
    with exec_tab5:
        ...
        if not shoonya_client.is_connected:
            ...
            with st.form("shoonya_order"):
                ...
                o2 = st.columns(3)
                with o2[0]:
                    ex = st.selectbox("Exchange", ["NSE", "BSE", "NFO"], key="trd_ex")
                with o2[1]:
                    ot = st.selectbox("Order Type", ["LIMIT", "MARKET", "SL", "SL-M"], key="trd_ot")
                with o2[2]:
                    vl = st.selectbox("Validity", ["DAY", "IOC"], key="trd_vl")
                ...
                if st.form_submit_button("🚀 Place Order", use_container_width=True):
                    ...
                    result = shoonya_client.place_order(sym, bs, qty, p, tr, ot, ex, vl)
                    if result and "order_id" in result:
                        st.success(f"✅ Placed! ID: {result['order_id']}")
                        add_trade(f"{sym}.NS", pr or 0, qty, bs,
                                  tags=["shoonya", "live"],
                                  notes=f"Auto-logged. Order: {result['order_id']}")
                    else:
