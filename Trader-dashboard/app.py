...
# =========================================================
    ...
    bt_tab1, bt_tab2, bt_tab3, bt_tab4 = st.tabs(["🔬 v6 Precision", "🏦 v5 Institutional", "🚀 v4 Precision", "⚖️ Comparison"])
    ...
    with bt_tab1:
        ...
        with st.expander("⚙️ v6 Configuration — Toggle Each Filter", expanded=False):
            v6e_col1, v6e_col2, v6e_col3 = st.columns(3)
            with v6e_col1:
            ...
            with v6e_col2:
                ...
                f_v6_chandelier = st.checkbox("Chandelier Trail (HH-3xATR)", True, key="v6_ch")
                f_v6_sellsig = st.checkbox("SellSig Exit", True, key="v6_ss")
            ...
            with v6e_col3:
                ...
                f_v6_earnings = st.checkbox("Earnings Blackout", True, key="v6_ern",
                                            help="No entry 5d before / 2d after earnings")
                f_v6_time = st.checkbox("Time Stop (20d, <-0.5R)", True, key="v6_ts")
                f_v6_costs = st.checkbox("SEBI Cost Model", True, key="v6_cost",
                                         help="STT, brokerage, exchange, GST, stamp duty")
        ...
        if st.button("🔬 Run v6 Backtest", key="v6_run", width='stretch', type="primary"):
            with st.spinner(f"Running v6 cost-aware backtest on {v6_ticker}..."):
                ...
                v6_config = {"capital": v6_cap}

                # Only pass filters if they're enabled — the Backtester will use defaults
                active_filters = []
                if f_v6_weekly: active_filters.append("weekly")
                if f_v6_adx: active_filters.append("adx")
                if f_v6_stack: active_filters.append("stack")
                if f_v6_vol: active_filters.append("volume")
                if f_v6_rs: active_filters.append("rs")
                if f_v6_earnings: active_filters.append("earnings")

                # We use the Backtester directly for toggle support
                from core.profit_engine_v6 import Backtester
                bt = Backtester(v6_config)
                result = bt.run(v6_ticker)
