...
if "navigate_ticker" not in st.session_state:
    st.session_state.navigate_ticker = None
...
def goto_chart(ticker):
    ...
    st.session_state.page_radio = "Chart"
    st.rerun()
...
def stock_link(symbol, display_name=None):
    ...
    Call this inside a columns/fragment where you want a clickable ticker.
    """
...
page = st.sidebar.radio(
    ...
    ["Dashboard", "Screener", "📈 Daily Trades", "📆 Swing Setups",
     "Chart", "Backtest", "Market Data", "Analytics", "Portfolio", "🔍 Stock Research"],
    index=0,
    key="page_radio"
)
...
# =========================================================
    ...
    # ══════════════════════════════════════════════
    with chart_tab1:
        # Use pre-selected ticker if navigating from stock link
        default_ticker = st.session_state.get("navigate_ticker", "RELIANCE.NS")
        if default_ticker == "RELIANCE.NS":
