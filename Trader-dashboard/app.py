...
# =========================================================
    ...
    bt_tab1, bt_tab2, bt_tab3, bt_tab4 = st.tabs(["🏦 v5 Institutional", "🚀 v4 Precision", "📊 v3 (Legacy)", "⚖️ Comparison"])

    # ═══════════════════════════════════════
    # TAB 1: PROFIT ENGINE v5 (INSTITUTIONAL)
    # ═══════════════════════════════════════
    with bt_tab1:
        ...
        with st.expander("⚙️ v5 Configuration — Toggle Each Filter", expanded=False):
            ...
            with s1_col1:
                ...
                f_vf = st.checkbox("Volume > 1.5x avg", True, key="v5_vf")
                f_rs = st.checkbox("RS vs Nifty > 1.0", True, key="v5_rs")
                f_52w = st.checkbox("Within 25% of 52w High", True, key="v5_52w")
            with s1_col2:
                f_stack = st.checkbox("EMA Stack (P>20>50>200)", True, key="v5_stack")
                f_vwap = st.checkbox("Above VWAP", True, key="v5_vwap")
                f_breadth = st.checkbox("Market Breadth > 1.2", True, key="v5_breadth")
            with s1_col3:
                st.markdown("**📤 Exit Rules**")
                f_chandelier = st.checkbox("Chandelier Exit", True, key="v5_ch")
                f_vol_ex = st.checkbox("Volume Exhaustion", True, key="v5_ve")
                f_gap = st.checkbox("Gap-Up Exit (>3%)", True, key="v5_gap")
            with s1_col4:
                f_time = st.checkbox("Time Stop (dead money)", True, key="v5_ts")
                st.markdown("**⚡ Risk Upgrades**")
                f_heat = st.checkbox("Portfolio Heat Limit", True, key="v5_heat")
                f_corr = st.checkbox("Correlation Filter", True, key="v5_corr")
                f_cb = st.checkbox("Circuit Breaker", True, key="v5_cb")
                f_kelly = st.checkbox("Dynamic Kelly", True, key="v5_dk")
                f_am = st.checkbox("Anti-Martingale", True, key="v5_am")
    ...
    # ═══════════════════════════════════════
    # TAB 2: PROFIT ENGINE v4 (PRECISION)
    # ═══════════════════════════════════════
    with bt_tab2:
            ...
            with st.spinner(f"Running {cmp_type} on {cmp_symbol}..."):
                ...
                comp = {
                    ...
                    "details": new_data
                        ...
                        "improvement": {
                            "PF Delta %": round((new.get("Profit Factor", 0) - old.get("Profit Factor", 0)) / max(old.get("Profit Factor", 0.1), 0.1) * 100, 1),
                            "Return Delta %": round(new.get("Total Return %", 0) - old.get("Total Return %", 0), 2),
                        }
                    }
