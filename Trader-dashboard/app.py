...
<style>
:root {{
    --bg-primary: {"#0a0e17" if _dark else "#f8fafc"};
    --bg-secondary: {"#111827" if _dark else "#f1f5f9"};
    --bg-card: {"rgba(17, 24, 39, 0.7)" if _dark else "rgba(255, 255, 255, 0.7)"};
    --bg-card-hover: {"rgba(31, 41, 55, 0.9)" if _dark else "rgba(255, 255, 255, 0.95)"};
    --text-primary: {"#f3f4f6" if _dark else "#0f172a"};
    --text-secondary: {"#9ca3af" if _dark else "#64748b"};
    --border-color: {"rgba(255, 255, 255, 0.08)" if _dark else "rgba(0, 0, 0, 0.08)"};
    --border-hover: {"rgba(240, 185, 11, 0.4)" if _dark else "rgba(217, 119, 6, 0.4)"};
    --accent-gold: {"#f0b90b" if _dark else "#d97706"};
    --accent-gold-glow: {"rgba(240, 185, 11, 0.2)" if _dark else "rgba(217, 119, 6, 0.2)"};
    --accent-green: {"#00ff9f" if _dark else "#059669"};
    --accent-red: {"#ff4d6d" if _dark else "#dc2626"};
    --accent-blue: {"#00bfff" if _dark else "#0284c7"};
}}
</style>
"""
...
# =========================================================
    ...
    with dcol1:
        daily_universe = st.selectbox(
            "Universe",
            ["Nifty 50", "Nifty 200 (359)", "Nifty 500", "Nifty 50 + 200 (409)"],
...
# =========================================================
    ...
    uni_map = {
        ...
        "Nifty 200 (359)": NIFTY200,
        "Nifty 500": NIFTY500,
        "Nifty 50 + 200 (409)": list(dict.fromkeys(NIFTY50 + NIFTY200)),
    }
...
# =========================================================
    ...
    with bt_tab1:
        ...
        if "v4_last_result" not in st.session_state:
            st.session_state.v4_last_result = None
        if "v4_last_ticker" not in st.session_state:
            st.session_state.v4_last_ticker = ""
        ...
        # ── Live Signal Monitor (after successful backtest) ──
        if st.session_state.get("v4_last_result") and st.session_state.v4_last_result.get("Total Trades", 0) > 0:
            _sym = st.session_state.v4_last_ticker
            st.markdown("---")
            st.markdown("### 🔴 LIVE SIGNAL — Is This Stock Ready NOW?")
            st.caption(f"Current V4 signal for {_sym.replace('.NS','')} based on latest market data")

            with st.spinner(f"Checking live signal for {_sym}..."):
                _df_sig = load_data(_sym, period="1y")
                if _df_sig is not None and len(_df_sig) > 200:
                    _df_sig = compute_indicators(_df_sig)
                    _latest = _df_sig.iloc[-1]
                    _sig = generate_v4_signal(_df_sig)

                    _c = _latest.get("Close", 0)
                    _atr = _latest.get("ATR", _c * 0.02)

                    # ── Signal verdict banner ──
                    if _sig.get("Signal") in ("STRONG_BUY", "BUY"):
                        st.success(f"### 🟢 {_sig['Signal']} — ENTRY SIGNAL ACTIVE")
                    elif _sig.get("Signal") == "SELL":
                        st.error(f"### 🔴 {_sig['Signal']} — AVOID / EXIT")
                    else:
                        st.info(f"### 🟡 {_sig['Signal']} — WAIT for better setup")

                    # ── Key levels & signal data ──
                    sc = st.columns(5)
                    with sc[0]:
                        st.metric("Signal Score", f"{_sig.get('Score', 0)}/10")
                    with sc[1]:
                        st.metric("Current Price", f"₹{_c:,.2f}")
                    with sc[2]:
                        st.metric("Stop Loss", f"₹{_sig.get('StopLoss', 0):,.2f}",
                                  delta=f"{(_c - _sig.get('StopLoss', _c)) / _c * 100:.1f}%" if _sig.get('StopLoss') else None)
                    with sc[3]:
                        st.metric("Target", f"₹{_sig.get('Target', 0):,.2f}",
                                  delta=f"{(_sig.get('Target', _c) - _c) / _c * 100:.1f}%" if _sig.get('Target') else None)
                    with sc[4]:
                        _rr = (_sig.get('Target', _c) - _c) / max(_c - _sig.get('StopLoss', _c), 0.01) if _sig.get('StopLoss') and _sig.get('Target') else 0
                        st.metric("R:R", f"1:{_rr:.2f}" if _rr > 0 else "—")

                    # ── Filter checklist ──
                    st.markdown("#### 📋 V4 Filter Checklist")
                    _rsi = _latest.get("RSI", 50)
                    _adx = _latest.get("ADX", 0)
                    _macd_h = _latest.get("MACD_Hist", 0)
                    _st_dir = _latest.get("SuperTrend_Dir", -1)
                    _rvol = _latest.get("RVOL", 1.0)
                    _ema20 = _latest.get("EMA20", _c)
                    _ema50 = _latest.get("EMA50", _c)
                    _ema200 = _latest.get("EMA200", _c)

                    filters = [
                        ("📈 Weekly Trend Gate", True, "Weekly trend is not bearish"),
                        ("📈 ADX > 22", _adx > 22, f"ADX = {_adx:.1f}"),
                        ("📊 RSI > 50", _rsi > 50, f"RSI = {_rsi:.1f}"),
                        ("📊 MACD Hist > 0", _macd_h > 0, f"MACD = {_macd_h:.4f}"),
                        ("🔺 SuperTrend UP", _st_dir == 1, f"SuperTrend = {'UP' if _st_dir == 1 else 'DOWN'}"),
                        ("📊 RVOL > 1.2", _rvol > 1.2, f"RVOL = {_rvol:.2f}x"),
                        ("📐 P > EMA20 > EMA50 > EMA200", _c > _ema20 > _ema50 > _ema200,
                         f"₹{_c:.0f} > ₹{_ema20:.0f} > ₹{_ema50:.0f} > ₹{_ema200:.0f}"),
                    ]

                    fcols = st.columns(4)
                    for i, (label, passed, detail) in enumerate(filters):
                        with fcols[i % 4]:
                            icon = "✅" if passed else "❌"
                            color = "#00ff9f" if passed else "#ff4d6d"
                            st.markdown(
                                f"<div style='background:#131722;border:1px solid #21262d;"
                                f"border-radius:6px;padding:8px 12px;margin:4px 0;'>"
                                f"<span style='font-size:13px;'>{icon} {label}</span><br>"
                                f"<span style='font-size:11px;color:{color};'>{detail}</span></div>",
                                unsafe_allow_html=True
                            )

                    # ── Action recommendation ──
                    st.markdown("---")
                    passed_count = sum(1 for _, p, _ in filters if p)
                    if _sig.get("Signal") in ("STRONG_BUY", "BUY") and passed_count >= 5:
                        st.success(f"✅ **ACTIONABLE** — {passed_count}/7 filters pass. "
                                   f"The strategy backtested profitably on this stock, and the current setup aligns. "
                                   f"Consider entering with proper risk management.")
                    elif passed_count >= 4:
                        st.info(f"💡 **WATCHLIST** — {passed_count}/7 filters pass. "
                                f"Wait for more confirmations before entering.")
                    else:
                        st.warning(f"⏳ **HOLD OFF** — Only {passed_count}/7 filters pass. "
                                   f"The stock is not ready yet. Monitor for setup improvement.")

        # Store last backtest result for live signal
        if 'result' in dir() and result and result.get("Total Trades", 0) > 0:
            st.session_state.v4_last_result = result
            st.session_state.v4_last_ticker = v4_ticker

    # ═══════════════════════════════════════════════════════════════
    # 📡 LIVE V4 UNIVERSE SCANNER — Auto-detect BUY signals
    # ═══════════════════════════════════════════════════════════════
    st.markdown("---")
    with st.expander("📡 Live V4 Universe Scanner — Find Stocks Ready NOW", expanded=False):
        st.caption("Scans entire universe for stocks currently giving V4 BUY/STRONG_BUY signals. "
                   "No backtest needed — uses live market data.")

        v4s_col1, v4s_col2, v4s_col3 = st.columns([2, 1, 1])
        with v4s_col1:
            v4s_uni = st.selectbox("Universe", ["Nifty 50", "Nifty 200 (359)", "Nifty 500", "Nifty 50 + 200 (409)"],
                                   index=0, key="v4s_uni")
        with v4s_col2:
            v4s_min_score = st.slider("Min Score", 1, 10, 4, 1, key="v4s_min", help="Minimum V4 signal score (0-10)")
        with v4s_col3:
            v4s_scan = st.button("🔍 Scan for Live Signals", key="v4s_scan", type="primary", use_container_width=True)
        ...
        if v4s_scan:
            ...
            v4s_symbols = uni_map.get(v4s_uni, NIFTY50)
            v4s_results = []
            v4s_prog = st.progress(0, text="Scanning...")

            for v4s_i, v4s_sym in enumerate(v4s_symbols):
                try:
                    v4s_prog.progress((v4s_i + 1) / len(v4s_symbols),
                                      text=f"Scanning {v4s_i+1}/{len(v4s_symbols)}: {v4s_sym}")
                    v4s_df = load_data(v4s_sym, period="1y")
                    if v4s_df is not None and len(v4s_df) > 200:
                        v4s_df = compute_indicators(v4s_df)
                        v4s_sig = generate_v4_signal(v4s_df)
                        if v4s_sig.get("Score", 0) >= v4s_min_score and v4s_sig.get("Signal") in ("BUY", "STRONG_BUY"):
                            v4s_last = v4s_df.iloc[-1]
                            v4s_atr = v4s_last.get("ATR", v4s_last["Close"] * 0.02)
                            v4s_rr = (v4s_sig.get("Target", v4s_last["Close"]) - v4s_last["Close"]) / \
                                     max(v4s_last["Close"] - v4s_sig.get("StopLoss", v4s_last["Close"]), 0.01)
                            v4s_results.append({
                                "Symbol": v4s_sym,
                                "Signal": v4s_sig["Signal"],
                                "Score": v4s_sig["Score"],
                                "Price": round(v4s_last["Close"], 2),
                                "Stop": v4s_sig.get("StopLoss", 0),
                                "Target": v4s_sig.get("Target", 0),
                                "R:R": round(v4s_rr, 2),
                                "RSI": round(v4s_last.get("RSI", 50), 1),
                                "ADX": round(v4s_last.get("ADX", 0), 1),
                                "RVOL": round(v4s_last.get("RVOL", 1), 2),
                            })
                except Exception:
                    continue
            ...
            if v4s_results:
                v4s_results.sort(key=lambda x: x["Score"], reverse=True)
                st.success(f"✅ Found {len(v4s_results)} stocks with V4 {v4s_min_score}+ signal in {v4s_uni}")

                # Display as clickable cards
                for v4s_r_idx, v4s_r in enumerate(v4s_results):
                    _sym = v4s_r["Symbol"]
                    _sig = v4s_r["Signal"]
                    _is_strong = _sig == "STRONG_BUY"
                    _border = "#00ff9f" if _is_strong else "#f0b90b"
                    _bg = "rgba(0,255,159,0.05)" if _is_strong else "rgba(240,185,11,0.05)"
                    _label = "🟢 STRONG BUY" if _is_strong else "🟡 BUY"

                    with st.container(border=True):
                        cols = st.columns([2, 1.5, 1.5, 1.5, 1.5, 1, 1])
                        with cols[0]:
                            st.markdown(f"**{_sym.replace('.NS','')}**")
                            st.caption(f"{_label} · Score: {v4s_r['Score']}/10")
                            stock_link(_sym, display_name="📈 Chart", key_suffix=f"v4s_{v4s_r_idx}")
                        with cols[1]:
                            st.metric("Price", f"₹{v4s_r['Price']:,.2f}")
                        with cols[2]:
                            st.metric("Stop", f"₹{v4s_r['Stop']:,.2f}" if v4s_r['Stop'] else "—",
                                      delta_color="inverse")
                        with cols[3]:
                            st.metric("Target", f"₹{v4s_r['Target']:,.2f}" if v4s_r['Target'] else "—")
                        with cols[4]:
                            st.metric("R:R", f"1:{v4s_r['R:R']}" if v4s_r['R:R'] > 0 else "—")
                        with cols[5]:
                            st.metric("RSI", v4s_r['RSI'])
                        with cols[6]:
                            st.metric("ADX", v4s_r['ADX'])
            else:
                st.warning(f"No V4 signals found in {v4s_uni} with score ≥ {v4s_min_score}. "
                           "Try a larger universe or lower the minimum score.")
