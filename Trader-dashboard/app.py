...
from core.profit_engine import (
    generate_high_probability_signal,
    run_profit_backtest, compare_old_vs_new
)
...
# =========================================================
    ...
    bt_tab1, bt_tab2 = st.tabs(["🚀 Profit Engine", "⚖️ Old vs New"])

    # ═══════════════════════════════════════
    # TAB 1: PROFIT ENGINE BACKTEST
    # ═══════════════════════════════════════
    with bt_tab1:
        ...
        st.caption("💡 **Profit Levers**: ATR Trailing Stop + Market Regime Filter + Volume Surge + Multi-Factor Entry")

        if st.button("🚀 Run Profit Backtest", key="bt_run", width='stretch'):
            with st.spinner(f"Running profit-optimized backtest on {ticker}..."):
                result = run_profit_backtest(
                    ticker, initial_capital=initial_capital, risk_pct=risk_pct,
                    max_hold_bars=max_hold, use_trailing_stop=True, use_regime_filter=True
                )
            ...
            if result and "Total Trades" in result and result["Total Trades"] > 0:
                ...
                if trades_df is not None and not trades_df.empty:
                    ...
                    trades_df["Result"] = trades_df["Result"].apply(lambda v: "🟢 WIN" if v == "WIN" else "🔴 LOSS")
    ...
    # ═══════════════════════════════════════
    # TAB 2: OLD VS NEW COMPARISON
    # ═══════════════════════════════════════
    with bt_tab2:
        st.markdown("### ⚖️ Old Baseline vs Profit Engine")
        st.caption("Compare naive EMA/RSI strategy against the enhanced Profit Engine")

        cmp_symbol = st.text_input("Symbol", value="RELIANCE.NS", key="cmp_sym")

        if st.button("⚖️ Compare Strategies", key="cmp_run", width='stretch'):
            with st.spinner(f"Running side-by-side comparison on {cmp_symbol}..."):
                comp = compare_old_vs_new(cmp_symbol)
            ...
            if comp and "old" in comp and "new" in comp:
                ...
                with cmpc[2]:
                    st.markdown("**Profit Engine 🚀**")
                    color = lambda v, o: "#00ff9f" if v > o else "#ff4d6d"
                    pf_color = "#00ff9f" if new["Profit Factor"] > old["Profit Factor"] else "#ff4d6d"
                    st.markdown(f":green[{new['Total Return %']:+.2f}%]")
                    st.markdown(f":green[{new['Win Rate %']:.1f}%]")
                    st.markdown(f":green[{new['Profit Factor']:.2f}]")
                    st.markdown(f":green[{new['Max Drawdown %']:.2f}%]")
                    st.markdown(f":green[{new['Sharpe Ratio']:.2f}]")
                    st.markdown(f":green[{new['Total Trades']}]")

                # Improvement summary
                st.markdown("---")
                st.markdown("#### 📈 Improvement Summary")
                impcols = st.columns(3)
                impcols[0].metric("Profit Factor Δ", f"{imp['Profit Factor Δ']:+.1f}%")
                impcols[1].metric("Return Δ", f"{imp['Return Δ']:+.1f}%")
                pf_new = new["Profit Factor"]
                impcols[2].metric("Result", "✅ Big Win" if pf_new > 2 else ("✅ Win" if pf_new > 1.3 else "⚠️ Needs Tuning"),
                                  delta=f"PF: {pf_new:.2f}")

                st.success(f"💡 **{imp['Notes']}**")
                st.info(
                    "**What improved:**\n"
                    "- ✅ **ATR Trailing Stops** — Let winners run, cut losers at 2.5x ATR\n"
                    "- ✅ **Volume Surge Filter** — Only enter on RVOL > 2.5x bars\n"
                    "- ✅ **Market Regime Filter** — No trades during bearish markets\n"
                    "- ✅ **Multi-Factor Entry** — 8-point scoring system vs binary check\n"
                    "- ✅ **Max Hold Period** — Exit after 15 days to avoid dead money\n"
                    "- ✅ **Risk Per Trade** — Dynamic position sizing (2% risk per trade)"
                )
            else:
