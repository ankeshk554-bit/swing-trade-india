...
from core.profit_engine_v8 import (
    run_v8_backtest, compare_v8_vs_old,
    net_pnl, is_earnings_blackout
)
...
# ── AI Chat Sidebar (available on every page) ──
...
with st.sidebar.container():
    ...
    # API key + provider in one row
    kc1, kc2 = st.columns([3, 1])
    with kc1:
        ai_key = st.text_input("API Key", type="password", value=st.session_state.ai_api_key,
                               key="ai_key_bot", label_visibility="collapsed",
                               placeholder="DeepSeek/OpenAI key")
    with kc2:
        ai_provider = st.selectbox("Provider", ["DS", "OA"], index=0,
                                   key="ai_prov_bot", label_visibility="collapsed")

    if ai_key != st.session_state.ai_api_key:
        st.session_state.ai_api_key = ai_key
        st.session_state.ai_messages = []

    ai_base_url = "https://api.deepseek.com" if ai_provider == "DS" else "https://api.openai.com/v1"

    # Page context
    pg_ctx = {
        "Dashboard": "Market Dashboard — Nifty pulse, FII/DII, VIX, breadth.",
        "Screener": "Stock Screener — scanning for breakouts, patterns, momentum.",
        "Chart": "Chart page — candlesticks with RSI, ADX, MACD panels.",
        "Backtest": "Backtest page — V4/V5/V6/V7 engines with full analytics.",
    ...
    # Chat input + send
    ai_q = st.chat_input(f"Ask about {page}...", key="ai_chat_bot")
...
RULES: Use ₹, use .NS symbols, be concise (under 150 words), educational, NO buy/sell advice."""
            ...
            try:
                ...
                with st.spinner("🤔"):
                    resp = req.post(f"{ai_base_url}/v1/chat/completions",
                        headers={"Authorization": f"Bearer {st.session_state.ai_api_key}"},
                        json={"model": "deepseek-chat" if ai_provider == "DS" else "gpt-4o-mini",
                              "messages": msgs, "max_tokens": 1024, "temperature": 0.3},
                        timeout=30)
                    resp.raise_for_status()
                    reply = resp.json()["choices"][0]["message"]["content"]
                    st.session_state.ai_messages.append({"role": "assistant", "content": reply})

                    # Show last reply inline
                    st.info(reply[:200] + ("..." if len(reply) > 200 else ""))
            except Exception as e:
                err = str(e)
                if "401" in err:
                    st.error("❌ Bad API key")
                elif "429" in err:
                    st.error("⏳ Rate limited")
                else:
                    st.error(f"❌ {err[:60]}")

    if st.session_state.ai_messages and st.button("🗑️ Clear chat", key="ai_clr_bot"):
        st.session_state.ai_messages = []
        st.rerun()
...
# =========================================================
    ...
    bt_tab1, bt_tab2, bt_tab3, bt_tab4, bt_tab5, bt_tab6 = st.tabs(["🏆 v8 Bulletproof", "🎯 v7 Precision", "🔬 v6 Cost-Aware", "🏦 v5 Institutional", "🚀 v4 Precision", "⚖️ Comparison"])

    # ═══════════════════════════════════════
    # TAB 1: PROFIT ENGINE v8 (BULLETPROOF)
    # ═══════════════════════════════════════
    with bt_tab1:
        st.markdown("### 🏆 v8 — V4 Made Bulletproof")
        st.caption("Breakeven Stop · SEBI Costs · Circuit Breaker · Partial 1.5R · Earnings Blackout")
        st.info("3 precise changes: removed time stop · added breakeven at 1R · added partial 40% at 1.5R")

        v8_col1, v8_col2 = st.columns([2, 1])
        with v8_col1:
            v8_ticker = st.text_input("Symbol", value="RELIANCE.NS", key="v8_ticker")
        with v8_col2:
            v8_cap = st.number_input("Capital (₹)", 10000, 10000000, 100000, 50000, key="v8_cap")

        with st.expander("⚙️ v8 Toggles", expanded=False):
            v8c1, v8c2 = st.columns(2)
            with v8c1:
                f_v8_be = st.checkbox("Breakeven Stop (1R)", True, key="v8_be",
                                      help="Stop moves to entry after 1R in favour")
                f_v8_cost = st.checkbox("SEBI Cost Model", True, key="v8_cost")
                f_v8_cb = st.checkbox("Circuit Breaker (4 cons losses)", True, key="v8_cb")
            with v8c2:
                f_v8_partial = st.checkbox("Partial 1.5R (40%)", True, key="v8_partial")
                f_v8_ern = st.checkbox("Earnings Blackout", True, key="v8_ern")

        if st.button("🏆 Run v8 Backtest", key="v8_run", width='stretch', type="primary"):
            with st.spinner(f"Running v8 on {v8_ticker}..."):
                result = run_v8_backtest(
                    v8_ticker, capital=v8_cap,
                    use_breakeven_stop=f_v8_be,
                    use_cost_model=f_v8_cost,
                    use_circuit_breaker=f_v8_cb,
                    use_partial_booking=f_v8_partial,
                    use_earnings_blackout=f_v8_ern,
                )
            ...
            if result and result.get("Total Trades", 0) > 0:
                ...
                with kcols[0]:
                    ...
                    st.metric("📊 Profit Factor", f"{pf:.2f}",
                              delta="✅ Excellent" if pf > 2 else ("✅ Good" if pf > 1.5 else "⚠️"),
                              delta_color="normal" if pf > 1.5 else "off")
                with kcols[1]:
                    sh = result.get("Sharpe Ratio", 0)
                    st.metric("🚀 Sharpe", f"{sh:.2f}",
                              delta="✅ Good" if sh > 1.5 else "⚠️",
                              delta_color="normal" if sh > 1.5 else "off")
                with kcols[2]:
                    st.metric("💰 Return", f"{result.get('Total Return %', 0):+.2f}%")
                with kcols[3]:
                    st.metric("🏆 Win Rate", f"{result.get('Win Rate %', 0):.1f}%")
                kcols2 = st.columns(4)
                with kcols2[0]: st.metric("📉 Max DD", f"{result.get('Max Drawdown %', 0):.2f}%")
                with kcols2[1]: st.metric("🎯 Expectancy", f"{result.get('Expectancy %', 0):+.2f}%")
                with kcols2[2]: st.metric("Trades", result.get("Total Trades", 0))
                ...
                if tdf is not None and not tdf.empty:
                    ...
                    st.download_button("📥 Export CSV", csv_data, f"{v8_ticker}_v8_trades.csv", "text/csv", key="dl_v8")
    ...
    # ═══════════════════════════════════════
    # TAB 2: PROFIT ENGINE v7 (PRECISION)
    # ═══════════════════════════════════════
    with bt_tab2:
    ...
    # ═══════════════════════════════════════
    # TAB 3: PROFIT ENGINE v6 (COST-AWARE)
    # ═══════════════════════════════════════
    with bt_tab3:
    ...
    # ═══════════════════════════════════════
    # TAB 4: PROFIT ENGINE v5 (INSTITUTIONAL)
    # ═══════════════════════════════════════
    with bt_tab4:
    ...
    # ═══════════════════════════════════════
    # TAB 5: PROFIT ENGINE v4 (PRECISION)
    # ═══════════════════════════════════════
    with bt_tab5:
