...
from core.news_feed import fetch_rss_feeds, get_stock_news, scan_stock_news
    ...
    if movers and movers.get("gainers"):
        ...
        with m_cols[0]:
            ...
            for i, g in enumerate(movers["gainers"]):
                ...
                with gcols[0]:
                    stock_link(sym, display_name=f"{i+1}. {sym.replace('.NS','')}")
                with gcols[1]:
                ...
                for row_i in range(chart_links_rows):
                    ...
                    for col_i in range(5):
                        ...
                        if idx < top_for_charts:
                            ...
                            with cols[col_i]:
                                stock_link(sym, display_name=f"📊 {sym.replace('.NS','')}")
...
# =========================================================
    ...
    if daily_picks:
                    ...
                    if not st.session_state.get("ai_api_key"):
                        ...
                        with st.spinner("🤔 Analyzing setup..."):
                            sys_p, usr_p = build_stock_analysis_prompt(
                                pick["Symbol"], pick, timeframe="intraday"
                            )
        ...
        st.warning("No high-probability setups found. Try expanding the universe or scanning later when markets are active.")
...
# =========================================================
            ...
            with st.container(border=True):
                ...
                if st.button("🤖 AI Analysis", key=ai_btn_key, use_container_width=True):
                    if not st.session_state.get("ai_api_key"):
                        ...
                        with st.spinner("🤔 Analyzing setup..."):
                            sys_p, usr_p = build_stock_analysis_prompt(
                                pick["Symbol"], pick, timeframe="swing"
                            )
                            msgs = [{"role": "system", "content": sys_p}, {"role": "user", "content": usr_p}]
                            reply = call_ai(
                                msgs,
    ...
    if not weekly_picks and not monthly_picks:
        st.warning("No swing setups found. Try a larger universe or a different timeframe.")
...
# =========================================================
    ...
    with chart_tab1:
        ...
        with c_col1:
            ...
            ticker = st.text_input("Symbol (e.g., RELIANCE.NS)", value=default_ticker, key="chart_ticker")
...
# =========================================================
    ...
    # ═══════════════════════════════════════
    # TAB 1: V4 PRECISION ENGINE (ONLY)
    # ═══════════════════════════════════════
    with bt_tab1:
        st.markdown("### 🚀 V4 Precision Engine")
        st.caption("ADX Filter · Weekly Trend Gate · Partial 1.5R · Kelly Sizing · Chandelier Trail · SEBI Costs")

        v4_col1, v4_col2 = st.columns([2, 1])
        with v4_col1:
        ...
        if st.button("🚀 Run V4 Backtest", key="v4_run", width='stretch', type="primary"):
            with st.spinner(f"Running V4 backtest on {v4_ticker}..."):
                result = run_v4_backtest(v4_ticker, capital=v4_cap)
            ...
            if result and result.get("Total Trades", 0) > 0:
                ...
                with kcols[2]:
                    st.metric("💰 Return", f"{m.get('Total Return %', 0):+.2f}%")
                with kcols[3]:
                ...
                with kcols2[3]: st.metric("Final Capital", f"₹{m.get('Final Capital', 0):,.0f}")
                ...
                if tdf is not None and not tdf.empty:
                    ...
                    st.download_button("📥 Export CSV", csv_data, f"{v4_ticker}_v4_trades.csv", "text/csv", key="dl_v4")
...
# =========================================================
    ...
    st.caption("Real-time RSS feeds from Indian & global financial sources · Auto-refreshes every 10 min")

    # ── Controls ──
    ncol1, ncol2, ncol3 = st.columns([2, 1, 1])
    with ncol1:
        news_filter = st.selectbox(
            "Filter", ["All News", "High Impact Only", "Stock-Specific"],
            index=0, key="news_filter"
        )
    with ncol2:
        news_refresh = st.button("🔄 Refresh Feed", key="news_refresh", type="primary", use_container_width=True)
    with ncol3:
        auto_refresh = st.checkbox("Auto-refresh", True, key="news_auto",
                                    help="Refreshes every 10 minutes")

    # ── Cache news in session state ──
    if "news_cache" not in st.session_state or "news_cache_time" not in st.session_state:
    ...
    if auto_refresh and st.session_state.news_cache_time:
        elapsed = (datetime.now() - st.session_state.news_cache_time).total_seconds()
    ...
    if should_refresh or st.session_state.news_cache is None:
        with st.spinner("Fetching live news feeds..."):
            ...
            st.session_state.news_cache = all_news
            st.session_state.news_cache_time = datetime.now()
    else:
        all_news = st.session_state.news_cache

    # ── Filter ──
    if news_filter == "High Impact Only":
        all_news = [n for n in all_news if n.get("impact", 0) >= 40]
    elif news_filter == "Stock-Specific":
        all_news = [n for n in all_news if n.get("stocks_mentioned")]

    if not all_news:
        st.info("No news items loaded. Click Refresh Feed to fetch.")
    else:
        st.caption(f"Showing {len(all_news)} items · Last refresh: {st.session_state.news_cache_time.strftime('%H:%M:%S')}")

        # ── High-impact banner ──
        high_impact = [n for n in all_news if n.get("impact", 0) >= 60]
        if high_impact:
            st.markdown("### 🚨 Market-Moving News")
            for item in high_impact[:3]:
                with st.container(border=True):
                    cols = st.columns([4, 1])
                    with cols[0]:
                        impact_label = "🔴 HIGH" if item["impact"] >= 70 else "🟡 MEDIUM"
                        st.markdown(f"**{item['title']}**")
                        st.caption(f"{item['source']} · {impact_label} Impact ({item['impact']}/100)")
                        if item.get("stocks_mentioned"):
                            st.markdown("💡 Stocks: " + ", ".join(f"`{s}`" for s in item["stocks_mentioned"]))
                    with cols[1]:
        ...
        # ── Stock-specific news alerts ──
        stock_news_alerts = [n for n in all_news if n.get("stocks_mentioned")]
        if stock_news_alerts and news_filter != "High Impact Only":
            st.markdown("### 📈 Stocks in News")
            for item in stock_news_alerts[:5]:
                with st.container(border=True):
                    cols = st.columns([4, 1])
                    with cols[0]:
                        st.markdown(f"**{item['title']}**")
                        stocks_str = ", ".join(
                            f"<a href='#' onclick='return false;' style='color:#FFD700;'>{s.replace('.NS','')}</a>"
                            for s in item["stocks_mentioned"]
                        )
                        st.markdown(f"🏢 Related: {stocks_str}", unsafe_allow_html=True)
                        st.caption(f"{item['source']} · Impact: {item['impact']}/100")
                    with cols[1]:
                        st.markdown(f"[🔗 Read]({item['link']})")
        ...
        for item in all_news[:30]:
            with st.container(border=True):
                impact_color = "🔴" if item["impact"] >= 60 else ("🟡" if item["impact"] >= 30 else "⚪")
                cols = st.columns([4, 1])
                with cols[0]:
                    st.markdown(f"**{item['title']}**")
                    st.caption(f"{item['source']} · {impact_color} {item['impact']}/100")
                    if item.get("stocks_mentioned"):
                        st.caption("📌 " + ", ".join(item["stocks_mentioned"]))
                with cols[1]:
                    st.markdown(f"[🔗 Open]({item['link']})")

    # ── Stock-specific news scanning ──
    st.markdown("---")
    with st.expander("🔍 Stock-Specific News Scanner", expanded=False):
        nsym = st.text_input("Enter symbol (e.g., RELIANCE.NS)", value="RELIANCE.NS", key="nsym")
        if st.button("🔍 Check News", key="nsym_btn"):
