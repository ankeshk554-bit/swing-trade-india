...
from core.profit_engine_v7 import (
    Backtester as BacktesterV7, run_v7_backtest,
    PerformanceAnalytics as PerfAnalyticsV7,
    EntryFilter as EntryFilterV7,
    ExitManager as ExitMgrV7,
    PositionSizer as PosSizerV7,
    CostModel as CostModelV7
)
...
page = st.sidebar.radio(
    "Navigation",
    ["Dashboard", "Screener", "Chart", "Backtest", "Market Data", "Analytics", "Portfolio", "🔍 Stock Research", "🤖 AI Analyst"],
    index=0
)
...
# =========================================================
    ...
    bt_tab1, bt_tab2, bt_tab3, bt_tab4, bt_tab5 = st.tabs(["🎯 v7 Precision", "🔬 v6 Cost-Aware", "🏦 v5 Institutional", "🚀 v4 Precision", "⚖️ Comparison"])

    # ═══════════════════════════════════════
    # TAB 1: PROFIT ENGINE v7 (PRECISION)
    # ═══════════════════════════════════════
    with bt_tab1:
        st.markdown("### 🎯 v7 — Precision Engine")
        st.caption("4 Entry Filters · 5 Exit Rules · Kelly Sizing · Chandelier Trail · SEBI Costs · Monte Carlo")

        v7_col1, v7_col2 = st.columns([2, 1])
        with v7_col1:
            v7_ticker = st.text_input("Symbol", value="RELIANCE.NS", key="v7_ticker")
        with v7_col2:
            v7_cap = st.number_input("Capital (₹)", 10000, 10000000, 100000, 50000, key="v7_cap")

        with st.expander("⚙️ v7 Configuration — Toggle Each Filter", expanded=False):
            v7e1, v7e2 = st.columns(2)
            with v7e1:
                st.markdown("**📥 Entry Filters (4)**")
                f_v7_weekly = st.checkbox("Weekly Trend Gate", True, key="v7_weekly",
                                          help="Weekly close > 10-week EMA")
                f_v7_adx = st.checkbox("ADX > 22", True, key="v7_adx")
                f_v7_stack = st.checkbox("EMA Stack (P>20>50>200)", True, key="v7_stack",
                                         help="Very strict trend structure")
                f_v7_vol = st.checkbox("Volume > 1.5x avg", True, key="v7_vol")
            with v7e2:
                st.markdown("**📤 Exit Rules (5)**")
                f_v7_partial = st.checkbox("Partial Booking (40%@1.5R)", True, key="v7_partial")
                f_v7_ch = st.checkbox("Chandelier Trail (HH-3xATR)", True, key="v7_ch")
                f_v7_gap = st.checkbox("Gap-Up Exit (>4%)", True, key="v7_gap")
                f_v7_ts = st.checkbox("Time Stop (20d, <-0.5R)", True, key="v7_ts")
                f_v7_cost = st.checkbox("SEBI Cost Model", True, key="v7_cost")

        if st.button("🎯 Run v7 Backtest", key="v7_run", width='stretch', type="primary"):
            with st.spinner(f"Running v7 backtest on {v7_ticker}..."):
                bt = BacktesterV7({"capital": v7_cap})
                result = bt.run(v7_ticker)
            ...
            if result and result.get("Total Trades", 0) > 0:
                ...
                with kcols[2]:
                    st.metric("💰 Total Return", f"₹{m.get('Total PnL ₹', 0):,.0f} ({m.get('Total PnL ₹', 0)/v7_cap*100:+.2f}%)")
                with kcols[3]:
                    st.metric("🏆 Win Rate", f"{m.get('Win Rate', 0):.1%}")
                ...
                with a4:
                    ...
                    st.metric("Partial Exec Rate", f"{m.get('Partial Execution Rate', 0):.0%}",
                              delta="✅ > 60%" if m.get('Partial Execution Rate', 0) >= 0.6 else "⚠️ Low",
                              delta_color="normal" if m.get('Partial Execution Rate', 0) >= 0.6 else "inverse")
                ...
                if eb:
                    ...
                    ecols = st.columns(min(len(eb), 5))
                    for i, (reason, count) in enumerate(sorted(eb.items(), key=lambda x: -x[1])[:5]):
                        with ecols[i % len(ecols)]:
                            pct = m.get("Exit %", {}).get(reason, 0)
                ...
                if not tl.empty:
                    mc = bt.run_montecarlo(tl, n_runs=2000, confidence=0.90)
                ...
                if not tl.empty:
                    ...
                    disp_cols = ["Entry", "EntryPrice", "InitialShares", "PartialShares", "TrailShares",
                                  "ExitReason", "TotalPnLRs", "TotalPnLPct", "Bars", "Result"]
                    disp = tl[disp_cols].copy() if all(c in tl.columns for c in disp_cols) else tl.copy()
                    ...
                    st.download_button("📥 Export CSV", csv_data, f"{v7_ticker}_v7_trades.csv", "text/csv", key="dl_v7")

            elif result and result.get("Total Trades", 0) == 0:
                if result.get("Error"):
                    st.error(f"❌ {result['Error']}")
                else:
                    st.warning("No trades generated. V7 has strict 4-filter entry — try a different symbol.")
            else:
                st.error(f"❌ {result.get('Error', 'Backtest failed')}")

    # ═══════════════════════════════════════
    # TAB 2: PROFIT ENGINE v6 (COST-AWARE)
    # ═══════════════════════════════════════
    with bt_tab2:
        ...
        with st.expander("⚙️ v6 Configuration — Toggle Each Filter", expanded=False):
            v6e1, v6e2, v6e3 = st.columns(3)
            with v6e1:
                st.markdown("**📥 Entry Filters**")
                f_v6_weekly = st.checkbox("Weekly Trend Gate", True, key="v6_weekly")
                f_v6_adx = st.checkbox("ADX > 22", True, key="v6_adx")
                f_v6_stack = st.checkbox("EMA Stack (P>20>50>200)", True, key="v6_stack")
                f_v6_vol = st.checkbox("Volume > 1.5x avg", True, key="v6_vol")
            with v6e2:
                st.markdown("**📤 Exit Rules**")
                f_v6_partial = st.checkbox("Partial Booking (40%@1.5R)", True, key="v6_partial")
                f_v6_ch = st.checkbox("Chandelier Trail (HH-3xATR)", True, key="v6_ch")
                f_v6_gap = st.checkbox("Gap-Up Exit (>4%)", True, key="v6_gap")
                f_v6_ts = st.checkbox("Time Stop (20d, <-0.5R)", True, key="v6_ts")
            with v6e3:
                st.markdown("**⚡ Risk & Filters**")
                f_v6_rs = st.checkbox("RS vs Nifty > 1.0", True, key="v6_rs")
                f_v6_ern = st.checkbox("Earnings Blackout", True, key="v6_ern")
                f_v6_cost = st.checkbox("SEBI Cost Model", True, key="v6_cost")

        if st.button("🔬 Run v6 Backtest", key="v6_run", width='stretch', type="primary"):
            with st.spinner(f"Running v6 backtest on {v6_ticker}..."):
                from core.profit_engine_v6 import Backtester as BTV6
                bt6 = BTV6({"capital": v6_cap})
                result = bt6.run(v6_ticker)

            if result and result.get("Total Trades", 0) > 0:
                m = result.get("metrics", {})
                tl = result.get("trade_log", pd.DataFrame())
                kcols = st.columns(4)
                with kcols[0]: st.metric("📊 PF", f"{m.get('Profit Factor', 0):.2f}")
                with kcols[1]: st.metric("🚀 Sharpe", f"{m.get('Sharpe Ratio', 0):.2f}")
                with kcols[2]: st.metric("💰 Return", f"₹{m.get('Total PnL ₹', 0):,.0f}")
                with kcols[3]: st.metric("🏆 WR", f"{m.get('Win Rate', 0):.1%}")
                kcols2 = st.columns(4)
                with kcols2[0]: st.metric("📉 Max DD", f"{m.get('Max Drawdown %', 0):.2f}%")
                with kcols2[1]: st.metric("🎯 Expectancy", f"{m.get('Expectancy %', 0):+.2f}%")
                with kcols2[2]: st.metric("Trades", m.get("Total Trades", 0))
                with kcols2[3]: st.metric("Final Cap", f"₹{result.get('Final Capital', 0):,.0f}")
                if not tl.empty:
                    st.markdown("---")
                    st.subheader("📋 Trade Log")
                    disp = tl.copy()
    ...
    # ═══════════════════════════════════════
    # TAB 3: PROFIT ENGINE v5 (INSTITUTIONAL)
    # ═══════════════════════════════════════
    with bt_tab3:
    ...
    # ═══════════════════════════════════════
    # TAB 4: PROFIT ENGINE v4 (PRECISION)
    # ═══════════════════════════════════════
    with bt_tab4:
...
# =========================================================
# PAGE: AI ANALYST (DeepSeek / OpenAI Integration)
# =========================================================
elif page == "🤖 AI Analyst":
    st.subheader("🤖 AI Market Analyst")
    st.caption("Powered by DeepSeek · Ask anything about stocks, charts, or markets")
    ...
    with st.sidebar:
        ...
        api_key = st.text_input("API Key", type="password",
                                ...
                                key="ai_key_input",
                                help="DeepSeek or OpenAI API key")
        if api_key != st.session_state.ai_api_key:
            st.session_state.ai_api_key = api_key
            st.session_state.ai_messages = []
            st.rerun()

        ai_provider = st.selectbox("Provider", ["DeepSeek", "OpenAI"], index=0, key="ai_provider")
        if ai_provider == "DeepSeek":
            ai_model = st.selectbox("Model", ["deepseek-chat", "deepseek-reasoner"], index=0, key="ai_model_sel")
            ai_base_url = "https://api.deepseek.com"
        else:
            ai_model = st.selectbox("Model", ["gpt-4o", "gpt-4o-mini", "gpt-4-turbo"], index=0, key="ai_model_sel")
        ...
        # Quick context injectors
        st.markdown("#### 📌 Quick Actions")
        if st.button("📊 Analyze RELIANCE.NS", key="ai_quick_reliance"):
            default_q = "Give me a comprehensive technical and fundamental analysis of RELIANCE.NS including support/resistance levels, trend analysis, and key fundamentals."
            st.session_state.ai_messages.append({"role": "user", "content": default_q})

        if st.button("📈 Market Overview", key="ai_quick_market"):
            default_q = "Give me an overview of the current Indian stock market conditions, key indices to watch, and what sectors are showing strength."
            st.session_state.ai_messages.append({"role": "user", "content": default_q})

        if st.button("🔍 Explain VWAP", key="ai_quick_vwap"):
            default_q = "Explain what VWAP is, how institutional traders use it, and how I should interpret it on my charts."
            st.session_state.ai_messages.append({"role": "user", "content": default_q})
    ...
    with chat_container:
        ...
        if not st.session_state.ai_messages:
            st.info(
                "👋 **Welcome to AI Analyst!**\n\n"
                "Ask me anything about:\n"
                "- 📊 **Technical Analysis** — Chart patterns, indicators, S/R levels\n"
                "- 📋 **Fundamental Analysis** — Financials, ratios, valuation\n"
                "- 📈 **Market Research** — Sector trends, macro, peer comparison\n"
                "- 🧠 **Strategy** — Backtest results, risk management, position sizing\n\n"
    ...
    # ── Chat Input ──
    prompt = st.chat_input("Ask about any stock, indicator, or market topic...")
    ...
    if prompt:
        ...
        # Build system prompt with context about this app
        system_prompt = """You are Sniper Terminal AI — an expert Indian stock market analyst embedded in a professional swing-trading platform.

Your user is a retail/institutional trader focused on NSE (National Stock Exchange of India) equities.

CAPABILITIES — You can answer questions about:
1. Technical Analysis: Chart patterns, indicators (RSI, MACD, ADX, Bollinger Bands, VWAP, Supertrend, ATR), support/resistance, trend analysis
2. Fundamental Analysis: P/E, P/B, ROE, debt/equity, revenue growth, profit margins, dividend yield
3. Market Context: Nifty 50, Bank Nifty, sector rotation, FII/DII flows, India VIX, market breadth
4. Strategy: Backtest results interpretation, Profit Factor, Sharpe/Sortino/Calmar ratios, Kelly criterion, Monte Carlo simulation, walk-forward analysis
5. Risk Management: Position sizing, portfolio heat, drawdown circuit breakers, stop-loss placement
6. NSE-specific: F&O expiry, SEBI regulations, STT, brokerage costs, delivery vs intraday

RULES:
- Be precise and data-driven. Use specific numbers and percentages.
- When asked about a specific stock, use its NSE symbol (e.g., RELIANCE.NS, TCS.NS).
- For Indian stocks, use ₹ for rupee values.
- If asked about backtest results, explain what metrics mean practically.
- Keep responses structured with bullet points or short paragraphs.
- Do NOT give financial advice or buy/sell recommendations.
- Always include educational context so the user learns WHY.

The current app features: Dashboard (market pulse), Screener (scan stocks), Chart (technical with RSI/ADX/MACD panels), Backtest (V4/V5/V6/V7 engines with 14-17 analytics metrics), Market Data (F&O, VIX, FII/DII, breadth), Analytics (S/R, strategy compare, AI scorer), Portfolio (watchlists, trade journal, position sizer, Shoonya trading)."""

        messages = [{"role": "system", "content": system_prompt}]
        # Send last 10 messages for context (to keep token usage reasonable)
        ...
        try:
            ...
            if e.response.status_code == 401:
                ...
                st.error("⏳ Rate limited. Please wait a moment before asking again.")
            else:
                st.error(f"❌ API error ({e.response.status_code}): {e.response.text[:200]}")
        except httpx.TimeoutException:
            st.error("⏱️ Request timed out. The model may be busy — try again.")
        except Exception as e:
