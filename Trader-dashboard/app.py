...
def stock_link(symbol, display_name=None, key_suffix=""):
    ...
    Use key_suffix to avoid duplicate key errors when same symbol appears
    in multiple sections (e.g., gainers AND volume).
    """
    ...
    if movers and movers.get("gainers"):
        ...
        with m_cols[0]:
            ...
            for i, g in enumerate(movers["gainers"]):
                ...
                with gcols[0]:
                    stock_link(sym, display_name=f"{i+1}. {sym.replace('.NS','')}", key_suffix="gain")
                with gcols[1]:
        ...
        with m_cols[1]:
            ...
            for i, g in enumerate(movers["losers"]):
                ...
                gcols = st.columns([3, 1])
                with gcols[0]:
                    stock_link(sym, display_name=f"{i+1}. {sym.replace('.NS','')}", key_suffix="loss")
                with gcols[1]:
        ...
        with m_cols[2]:
            ...
            for i, g in enumerate(movers["most_volume"]):
                ...
                gcols = st.columns([3, 1])
                with gcols[0]:
                    stock_link(sym, display_name=f"{i+1}. {sym.replace('.NS','')}", key_suffix="vol")
                with gcols[1]:
