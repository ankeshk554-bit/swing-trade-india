...
# =========================================================
    ...
    bt_tab1, bt_tab2, bt_tab3 = st.tabs(["🚀 Profit Engine v4", "📊 v3 (Legacy)", "⚖️ Old vs New"])

    # ═══════════════════════════════════════
    # TAB 1: PROFIT ENGINE v4 (INSTITUTIONAL)
    # ═══════════════════════════════════════
    with bt_tab1:
    ...
    with bt_tab3:
        ...
        st.caption("Compare Old Baseline, Profit Engine v3, and v4 side-by-side")

        cmp_type = st.radio("Comparison Type", ["v4 vs Old", "v3 vs Old", "v4 vs v3"],
        ...
        if st.button("⚖️ Compare", key="cmp_run", width='stretch'):
            if cmp_type == "v4 vs Old":
                with st.spinner(f"Running v4 vs Old on {cmp_symbol}..."):
                    comp = compare_v4_vs_old(cmp_symbol)
            elif cmp_type == "v3 vs Old":
                with st.spinner(f"Running v3 vs Old on {cmp_symbol}..."):
                    comp = compare_v3_vs_old(cmp_symbol)
            else:  # v4 vs v3
                from core.backtest import run_backtest as old_bt
                with st.spinner(f"Running v4 vs v3 on {cmp_symbol}..."):
                    new = run_v4_backtest(cmp_symbol)
                    old = run_v3_backtest(cmp_symbol)
                    comp = {
                        "old": {
