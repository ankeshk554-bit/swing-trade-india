        ...
        if st.button("🔗 Connect Shoonya", key="sh_connect"):
            if shoonya_uid and shoonya_pwd and shoonya_2fa:
                with st.spinner("Connecting..."):
                    result = shoonya_connect(shoonya_uid, shoonya_pwd, shoonya_2fa)
                    if isinstance(result, dict) and result.get("success"):
                        ...
                        st.error(f"❌ {err_msg}")
                        if "SEBI" in err_msg or "static" in err_msg.lower() or "whitelist" in err_msg.lower():
                            st.info(
                                "🔒 **SEBI Static URL Requirement**: Trading APIs in India require "
                                "registered/whitelisted URLs. Cloud-hosted apps (like Streamlit Cloud) "
                                "may be blocked.\n\n"
                                "**Solutions:**\n"
                                "1. Run the app **locally** — `streamlit run app.py`\n"
                                "2. Contact **Finvasia support** to whitelist your app URL\n"
                                "3. Use the app for **data only** via Yahoo Finance (slower but no auth needed)"
                            )
            else:
