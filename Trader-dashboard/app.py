...
with st.sidebar.expander("🤖 AI Chat", expanded=False):
    ai_key = st.text_input("API Key", type="password", value=st.session_state.ai_api_key,
                           key="ai_key_floating", label_visibility="collapsed",
                           placeholder="Enter DeepSeek/OpenAI key")
    if ai_key != st.session_state.ai_api_key:
        st.session_state.ai_api_key = ai_key
        st.session_state.ai_messages = []

    ai_provider = st.selectbox("Provider", ["DeepSeek", "OpenAI"], index=0,
                               key="ai_prov_floating", label_visibility="collapsed")
    ai_base_url = "https://api.deepseek.com" if ai_provider == "DeepSeek" else "https://api.openai.com/v1"

    # Page-aware context
    current_page = page
    page_contexts = {
        "Dashboard": "The user is on the Market Dashboard — showing Nifty pulse, FII/DII flows, VIX, market breadth.",
        "Screener": "The user is on the Stock Screener — scanning for breakout/pattern/momentum stocks.",
        "Chart": "The user is on the Chart page — viewing candlestick charts with RSI, ADX, MACD panels.",
        "Backtest": "The user is on the Backtest page — running V4/V5/V6/V7 backtest engines.",
        "Market Data": "The user is on Market Data — viewing F&O chain, VIX, FII/DII, sector rotation.",
        "Analytics": "The user is on Analytics — S/R levels, strategy comparison, AI scorer, Fibonacci.",
        "Portfolio": "The user is on Portfolio — watchlists, trade journal, position sizer, Shoonya live trading.",
        "🔍 Stock Research": "The user is on Stock Research — searching stocks for fundamentals, technicals, peers.",
    }
    page_hint = page_contexts.get(current_page, "The user is browsing the trading platform.")

    # Chat messages (compact, scrolling)
    chat_height = 200
    chat_html = '<div style="height:200px;overflow-y:auto;display:flex;flex-direction:column;gap:4px;">'
    for msg in st.session_state.ai_messages[-6:]:  # Show last 6 messages
        role_label = "🧑" if msg["role"] == "user" else "🤖"
        short = msg["content"][:100] + "..." if len(msg["content"]) > 100 else msg["content"]
        chat_html += f'<div style="font-size:12px;padding:4px 8px;background:#1a1f2e;border-radius:6px;"><b>{role_label}</b> {short}</div>'
    chat_html += '</div>'
    st.markdown(chat_html, unsafe_allow_html=True)

    ai_query = st.chat_input("Ask about markets...", key="ai_float_input")

    if ai_query:
        if not st.session_state.ai_api_key:
            st.error("Enter API key above first")
        else:
            st.session_state.ai_messages.append({"role": "user", "content": ai_query})
            system_prompt = f"""You are Sniper Terminal AI — an Indian stock market analyst inside a professional swing-trading platform.

CURRENT PAGE: {current_page}
CONTEXT: {page_hint}

You can answer about: technical analysis (RSI, MACD, ADX, BB, VWAP, Supertrend, ATR), fundamentals (P/E, P/B, ROE, D/E), NSE market (Nifty, VIX, FII/DII, F&O), strategy metrics (PF, Sharpe, Sortino, Calmar, Kelly, Monte Carlo).

RULES: Be precise, use ₹ for rupee values, use NSE symbols (.NS), give educational context, do NOT give buy/sell advice. Keep answers concise (under 200 words unless asked for detail)."""
            ...
            try:
                import httpx
                with st.spinner("🤔"):
                    resp = httpx.post(f"{ai_base_url}/v1/chat/completions",
                        headers={"Authorization": f"Bearer {st.session_state.ai_api_key}", "Content-Type": "application/json"},
                        json={"model": "deepseek-chat" if ai_provider == "DeepSeek" else "gpt-4o-mini",
    ...
    if st.button("🗑️ Clear", key="ai_clear_float"):
        st.session_state.ai_messages = []
        st.rerun()
