...
class Backtester:
    ...
    def run(self, symbol: str, start_date=None, end_date=None) -> dict:
            ...
            if pos and active_trade is not None:
                ...
                if decision["exit"]:
                    ...
                    if reason == "Partial40%":
                        ...
                        pnl_pct_adj = pnl_amt / (active_trade.entry_price * shares_exit) * 100
                        cap += pnl_amt
                    ...
                    if partial:
                        ...
                        pnl_amt -= cost["total_cost"]
                        cap += pnl_amt
                        pnl_pct_adj = pnl_amt / (active_trade.entry_price * total_shares) * 100
                        rm.close_position(f"T{trade_id}", pnl_pct_adj, pnl_amt)
                        trades.append({
                            "Entry": active_trade.entry_date, "Exit": date_ts,
                            "Entry₹": round(active_trade.entry_price, 2),
        ...
        if pos and active_trade is not None:
            ...
            pnl_amt -= cost["total_cost"]
            pnl_pct_adj = pnl_amt / (active_trade.entry_price * shares) * 100
            cap += pnl_amt
            trades.append({
