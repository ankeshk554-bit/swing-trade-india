...
class Backtester:
    ...
    def run(self, symbol: str, start_date=None, end_date=None) -> dict:
        ...
        for i in range(220, len(df)):
            ...
            if pos and active_trade is not None:
                ...
                if decision["exit"]:
                    ...
                    partial = decision.get("partial_booked", False)
                    shares_exit = decision.get("shares", active_trade.shares)
                    remaining = decision.get("remaining", 0)

                    # If partial booking, update trade state and continue
                    if reason == "Partial40%":
                        active_trade.partial_booked = True
                        active_trade.remaining_shares = remaining
                        active_trade.chandelier_stop = active_trade.entry_price

                        # Record partial trade
                        pnl_pct = (exit_price - active_trade.entry_price) / max(active_trade.entry_price, 0.01) * 100
                        pnl_amt = shares_exit * (exit_price - active_trade.entry_price)
                        # Apply costs
                        cost = IndiaMarketUtils.apply_cost_model(active_trade.entry_price, exit_price, shares_exit)
                        pnl_amt -= cost["total_cost"]
                        pnl_pct_adj = pnl_amt / max(active_trade.entry_price * shares_exit, 0.01) * 100
                        cap += pnl_amt
                        rm.close_position(f"T{trade_id}", pnl_pct_adj, pnl_amt)
                        trades.append({
                            "Entry": active_trade.entry_date, "Exit": date_ts,
                            "Entry₹": round(active_trade.entry_price, 2),
                            "Exit₹": round(exit_price, 2),
                            "PnL%": round(pnl_pct_adj, 2),
                            "PnL₹": round(pnl_amt, 2),
                            "Result": "WIN" if pnl_pct_adj > 0 else "LOSS",
                            "Bars": i - 220, "Reason": "Partial40%", "Shares": shares_exit,
                        })
                        continue

                    # Full exit
                    if partial:
                        # Already booked partial - this exit is for remaining shares
                        total_shares = active_trade.shares
                        total_pnl_amt = 0
                        # Partial was already recorded; now record trail exit of remaining
                        pnl_pct = (exit_price - active_trade.entry_price) / max(active_trade.entry_price, 0.01) * 100
                        pnl_amt = shares_exit * (exit_price - active_trade.entry_price)
                        cost = IndiaMarketUtils.apply_cost_model(active_trade.entry_price, exit_price, shares_exit)
                        pnl_amt -= cost["total_cost"]
                        cap += pnl_amt
                        pnl_pct_adj = pnl_amt / max(active_trade.entry_price * total_shares, 0.01) * 100
                        rm.close_position(f"T{trade_id}", pnl_pct_adj, pnl_amt)
                        trades.append({
                            "Entry": active_trade.entry_date, "Exit": date_ts,
                            "Entry₹": round(active_trade.entry_price, 2),
                            ...
                            "PnL₹": round(pnl_amt, 2),
                            "Result": "WIN" if pnl_pct_adj > 0 else "LOSS",
                            "Bars": i - 220, "Reason": reason, "Shares": shares_exit,
                        })
                    else:
                        pnl_pct = (exit_price - active_trade.entry_price) / max(active_trade.entry_price, 0.01) * 100
                        pnl_amt = shares_exit * (exit_price - active_trade.entry_price)
        ...
        if pos and active_trade is not None:
            ...
            pnl_pct = (fp - active_trade.entry_price) / max(active_trade.entry_price, 0.01) * 100
            pnl_amt = shares * (fp - active_trade.entry_price)
            cost = IndiaMarketUtils.apply_cost_model(active_trade.entry_price, fp, shares)
            pnl_amt -= cost["total_cost"]
            pnl_pct_adj = pnl_amt / max(active_trade.entry_price * shares, 0.01) * 100
            cap += pnl_amt
            trades.append({
                "Entry": active_trade.entry_date, "Exit": df.index[-1],
                "Entry₹": round(active_trade.entry_price, 2),
                "Exit₹": round(fp, 2),
                "PnL%": round(pnl_pct_adj, 2),
                "PnL₹": round(pnl_amt, 2),
                "Result": "WIN" if pnl_pct_adj > 0 else "LOSS",
                "Bars": len(df) - 220, "Reason": "EndOfData", "Shares": shares,
            })
