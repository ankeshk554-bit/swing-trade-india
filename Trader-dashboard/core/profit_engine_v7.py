...
class ExitManager:
    ...
    def get_exit(self, bar_df: pd.DataFrame, current_bar: int, trade, atr: float) -> tuple:
        ...
        # ── PRIORITY 2: Partial 40% at 1.5R (check BEFORE stop to improve partial rate)
        # Bug 2 fix: check partial trigger first so it has a chance to fire
        # before the stop hits on volatile bars
        ...
        # ── PRIORITY 3: Initial Stop ──
        if trade.initial_stop is not None and lo <= trade.initial_stop:
            exit_price = max(trade.initial_stop, lo * 0.99)
...
class PositionSizer:
    ...
    @staticmethod
    def shares(position_budget: float, entry_price: float, stop_price: float,
               vol_adj: float, streak_adj: float) -> int:
        """
        position_budget = capital * kelly_fraction (passed from caller)
        shares = floor(position_budget * vol_adj * streak_adj / (entry_price - stop_price))
        """
        risk_per_share = abs(entry_price - stop_price)
        if risk_per_share <= 0:
            return 1
        risk_amount = position_budget * vol_adj * streak_adj
    ...
    def run(self, symbol: str) -> dict:
        ...
        for i in range(220, len(df)):
            ...
            if not pos:
                ...
                shares = PositionSizer.shares(capital * kelly_f, c, initial_stop, vol_adj, s_adj)
                if shares < 1:
                    continue
            ...
            if pos and active_trade is not None:
                ...
                if reason == "Partial40%":
                    ...
                    active_trade.initial_stop = active_trade.entry_price  # Bug 3 fix: update initial_stop to breakeven too
