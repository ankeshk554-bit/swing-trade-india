...
IMPORTANT - SEBI Static URL Requirement:
  SEBI mandates that trading APIs must use a registered/whitelisted URL.
  Streamlit Cloud URLs (like *.streamlit.app) may NOT be whitelisted.
  If connection fails with "Invalid credentials" or "Connection refused",
  you need to:
    1. Register your app's URL with Finvasia support
    2. Or run the app locally (the API works from your home IP)
    3. Contact Finvasia to whitelist your Streamlit Cloud URL
...
class ShoonyaClient:
    ...
    def connect(self, uid: str, password: str, twofa: str, vendor_code: str = "") -> bool:
        """
        ...
        Args:
            ...
            twofa: Two-factor auth (PAN in UPPERCASE + DOB in DDMMYYYY format,
                   or TOTP token from the Shoonya mobile app)
            vendor_code: Optional vendor code (provided by Finvasia)
        ...
        try:
            ...
            try:
                ...
                logger.error(
                    "shoonya package not installed. Run: pip install shoonya"
                )
                return False
        ...
        with self._connection_lock:
            try:
                ...
                if ret and ret.get("stat") == "Ok":
                    ...
                    if "IP" in err or "ip" in err or "URL" in err or "url" in err:
                        logger.error(
                            "SEBI requires static URL whitelisting. "
                            "Cloud-hosted apps may be blocked. "
                            "Contact Finvasia support to whitelist your app URL, "
...
def connect(uid: str = None, password: str = None, twofa: str = None) -> dict:
    """
    ...
    try:
        ...
        return {"success": False, "error": "shoonya package not installed. Run: pip install shoonya"}
    ...
    return {"success": False, "error": "Connection failed. Check credentials. If credentials are correct, this may be due to SEBI static URL requirement — contact Finvasia support to whitelist this app's URL."}
