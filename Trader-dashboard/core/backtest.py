    ...
    try:
        ...
        for i in range(200, len(df)):
            ...
            if signal_info["Signal"] == "BUY" and not in_position:
                ...
                entry_date = row.name
                position = capital / max(entry_price, 0.01)
                in_position = True

            elif signal_info["Signal"] == "SELL" and in_position:
                exit_price = float(row["Close"])
                exit_date = row.name
                pnl = (exit_price - entry_price) / max(entry_price, 0.01) * 100
        ...
        # --- Compute Metrics ---
        total_return_pct = ((capital - initial_capital) / max(initial_capital, 1)) * 100
        ...
        df["Drawdown"] = (df["Equity"] - running_max) / running_max.replace(0, 1) * 100
