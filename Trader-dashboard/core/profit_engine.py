...
def generate_high_probability_signal(df, regime="BULLISH"):
    ...
    if rvol > 2.5:
        ...
        entry_score += 1
        reasons.append(f"Volume {rvol:.1f}x avg")
    else:
        entry_score -= 1
        warnings.append(f"Low volume {rvol:.1f}x")
    ...
    if recent.get("SUPERTREND_DIR", 0) == 1:
        ...
        entry_score -= 1
        warnings.append("Supertrend bearish")
    ...
    if entry_score >= 7:
        ...
        signal = "BUY"
    elif entry_score >= 3 and warnings:
        signal = "WEAK_BUY"

    if exit_score >= 3 and entry_score <= 2:
        signal = "SELL"
...
def run_profit_backtest(symbol, initial_capital=100000, risk_pct=2.0,
    ...
    try:
        ...
        for i in range(200, len(df)):
            ...
            # ─── ENTRY LOGIC ───
            if not in_position and sig["Signal"] in ("STRONG_BUY", "BUY"):
                # Apply regime filter
...
def compare_old_vs_new(symbol: str) -> dict:
    ...
    old_pf = old_result.get("Profit Factor", 1)
    new_pf = new_result.get("Profit Factor", 1)
    pf_improvement = round((new_pf - old_pf) / max(old_pf, 0.1) * 100, 1)

    old_ret = old_result.get("Total Return %", 0)
    new_ret = new_result.get("Total Return %", 0)
    ret_improvement = round(new_ret - old_ret, 1)

    return {
        "old": {
            "Total Return %": old_result.get("Total Return %", 0),
            "Win Rate %": old_result.get("Win Rate %", 0),
            "Profit Factor": old_result.get("Profit Factor", 0),
            "Max Drawdown %": old_result.get("Max Drawdown %", 0),
            "Sharpe Ratio": old_result.get("Sharpe Ratio", 0),
            "Total Trades": old_result.get("Total Trades", 0),
        },
        "new": {
            "Total Return %": new_result["Total Return %"],
            "Win Rate %": new_result["Win Rate %"],
            "Profit Factor": new_result["Profit Factor"],
            "Max Drawdown %": new_result["Max Drawdown %"],
            "Sharpe Ratio": new_result["Sharpe Ratio"],
            "Total Trades": new_result["Total Trades"],
        },
        "improvement": {
