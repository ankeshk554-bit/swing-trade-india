...
class EntryFilters:
    ...
    def relative_strength(symbol, lookback=63):
        ...
        try:
            ...
            sr = (float(stock["Close"].iloc[-1]) / float(stock["Close"].iloc[0])) - 1
            nr = (float(nifty["Close"].iloc[-1]) / float(nifty["Close"].iloc[0])) - 1
    ...
    def market_breadth_gate(min_ad_ratio=1.2):
        ...
        try:
            ...
            ret = (float(nifty["Close"].iloc[-1]) / float(nifty["Close"].iloc[-2]) - 1) * 100
...
class ExitRules:
    ...
    def gap_up_exit(df, current_bar, entry_price, pct_threshold=3.0):
        ...
        if gap_pct > pct_threshold:
            pnl = (curr_open - entry_price) / entry_price * 100
            if pnl > 0:
...
class RiskManager:
    ...
    def can_trade(self, current_bar: int, sector: str = None) -> tuple:
        ...
        # Max daily loss
        daily_loss = (self.daily_pnl - self.daily_pnl_start) / self.daily_pnl_start * 100
...
def compute_advanced_metrics(trades: list, initial_capital: float) -> dict:
    ...
    total_return = (net_profit + initial_capital) / initial_capital - 1
    ...
    for i in range(200, len(df)):
        ...
        if not pos and score >= 4:
            ...
            if c <= ep - atr * 2 or c >= ep + atr * 5 or score < 2:
                ...
                pos = False
    if pos:
        trades.append((float(df["Close"].iloc[-1]) - ep) / ep * 100)
    return trades
...
def parameter_stability_test(symbol: str, base_params: dict, ranges: dict) -> dict:
    ...
    for param, values in ranges.items():
        ...
        for val in values:
            ...
            if base_pf > 0:
                change = abs(pf - base_pf) / base_pf * 100
                if change > 30:
                    degraded = True
