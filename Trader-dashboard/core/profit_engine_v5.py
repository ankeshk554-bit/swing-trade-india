...
def run_v5_backtest(
    ...
    max_hold=15,
    use_regime=True,
    # ── Section 1: Entry Filters (all OFF by default — v4-style leniency) ──
    ...
    # ── Section 2: Exit Rules (minimal by default — v4-style basic trail) ──
    use_time_stop=True,
    use_chandelier=False,
    use_par_sar=False,
    use_vol_exhaust=False,
    use_gap_up_exit=False,
    # ── Section 3: Risk Upgrades (all OFF by default — fixed % sizing like v4) ──
        ...
        for i in range(200, len(df)):
            ...
            if not pos:
                ...
                if sig["Signal"] not in ("STRONG_BUY", "BUY"):
                    continue
                if sig["Score"] < (5 if regime == "BEARISH" else 3):
                    continue
